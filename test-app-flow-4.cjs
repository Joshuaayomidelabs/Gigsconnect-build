...
(async () => {
  const email = 'test_flow4_' + Date.now() + '@example.com';
  ...
  await supabaseAuth.from('subscriptions').insert({
    user_id: userId,
    plan_id: proPlan.id,
    plan_name: 'pro',
    status: 'active',
  ...
  const { data, error } = await supabaseAuth.from('subscriptions').select('*').eq('user_id', userId);
