...
(async () => {
  const email = 'test_get_sub_' + Date.now() + '@example.com';
  ...
  const res = await supabase.from('subscriptions').select('*').eq('user_id', userId);
  console.log('Select my sub:', res.data, res.error);
})();
