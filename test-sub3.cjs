...
(async () => {
  const email = 'test_sub3_' + Date.now() + '@example.com';
  ...
  console.log('Insert without select Error:', createErr);
  
  const { data: getSub, error: getErr } = await supabase.from('subscriptions').select('*').eq('user_id', userId);
  console.log('Select Sub:', getSub, 'Error:', getErr);
})();
