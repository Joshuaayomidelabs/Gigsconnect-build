...
(async () => {
  const email = 'test_bypass2_' + Date.now() + '@example.com';
  ...
  const { error: convErrorNoSelect } = await supabase.from('conversations').insert({});
  console.log('Insert conversation NO SELECT:', convErrorNoSelect);
