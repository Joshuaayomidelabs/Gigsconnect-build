...
(async () => {
  const email = 'test_flow7_' + Date.now() + '@example.com';
  ...
  const { error: insErr1 } = await supabaseAuth.from('subscriptions').insert({
    user_id: userId,
    plan_id: proPlan.id,
    plan_name: 'pro',
    status: 'active',
  ...
  console.log('Insert 1 Error:', insErr1);
  
  const { error: insErr2 } = await supabaseAuth.from('subscriptions').insert({
    user_id: userId,
    plan_id: proPlan.id,
    plan_name: 'pro',
    status: 'active',
  ...
  console.log('Insert 2 Error:', insErr2);
})();
