...
(async () => {
  ...
  // Try using global headers explicitly instead of setSession
  const supabaseAuth = createClient(process.env.VITE_SUPABASE_URL, process.env.VITE_SUPABASE_ANON_KEY, {
    global: { headers: { Authorization: `Bearer ${session.access_token}` } }
  });

  const res = await supabaseAuth.from('subscriptions').select('*');
  console.log('Select with explicit JWT:', res.data, res.error);
})();
